...
| 2026-07-21 | **About page real photos in.** Wired Bri's uploaded backyard family photo into the **Family Photo** banner (`.photo-placeholder`, 16/7, `object-position:center 28%` so all three faces stay in frame; file renamed to `bri-family.jpg` (was the camera name `E8692738-…​.jpg`)) and set the **intro portrait** to the existing `bri-hero.jpg` (was the missing `bri-about.jpg` placeholder; framed 4/5 card, `object-position:center 20%`). Both verified — no faces cropped. |
...
| 2026-09-08 | **Critique fixes — one title, ship-the-code story, Hub repaired.** Headline title is now **AI-Native Senior Product Designer** on every surface (homepage `<title>`/OG/meta, résumé hero + current-role row, About role line, Contact roles, GCS + Aimee kickers, Brix KB `level`/`work history`/`who is`/`about`/`resume`/`contact` answers); Design Engineer stays only as a Contact role tag + the DE lens chip. Copy everywhere now leads with *designs and ships full production code* — React, Git/CI, Cloudflare, API integrations, LLM wikis (résumé skill tags gained `Cloudflare (Workers, Pages)`, `APIs (REST, Anthropic)`, `LLM Wikis / Knowledge Bases`). **Hub case study repaired:** the four experience images pointed at a nonexistent `images/Financial Finesse Case Study/*.webp`; they now load from `images/Financial Finesse Hub Case Study/{3,4,5,6}. *.webp` (new WebP renders of the tracked PNGs, 6.5 MB → 1.4 MB, intrinsic dims updated). Both case-study videos re-encoded (H.264 crf 27, faststart): Hub welcome 23 MB → 10 MB, GCS demo 15 MB → 5.4 MB. **Work order** is now GCS → Brix → Aimee → Hub (Work grid, `CASE_STUDIES`, and the Next: loop); Work H1 → "AI products, designed and shipped." **Brix:** new `github` KB entry (routes "can I see her code") with a `block` link to the public repo — `serveEntry` now renders `entry.block`; the code exhibit (`#engine-excerpt`) is the **verbatim** `retrieve()` instead of a simplified copy; `tests` answer names the probe + browser audit; Aimee answers match the case study (coded prototypes + component library, engineering shipped 1:1 in Next.js); PD lens chip relabeled "AI Product Designer". GitHub link added to the rail footer, Contact page, and résumé contact row. Résumé page: "saved millions" → "avoided ~$2.4M". About experience entry rewritten to match the résumé timeline (2025– / 2017–2025). Probe: 37/38, 0 regressions. Not touched: the résumé PDF (regenerate separately), Aimee case-study body copy, the GCS numbers. |
| 2026-09-08 | **Positioning brief applied (docs/positioning-brief-for-claude-code.md).** Title everywhere is **AI-Native Senior Product Designer**; second line everywhere is **I design the product, then I ship the code.** "Design Engineer" survives only as the Contact second line, a skills group, and the Brix answer. **Homepage:** the empty state is now a hero (name, title, second line, View the code ↗ / Global Content Studio → / Résumé ↓) above the starter cards; the role-lens row, its KB entries, and `lens_de`/`lens_pd` are gone; the "See all case studies" card became "See the code she ships" (opens `/code/`); rail has a **Code** page entry; thinking delay is one 120ms beat; FUNNY is three plain lines and the fallback always offers nearest topics; engineering words (broke/fix/tradeoff/architecture/test/why) never fall back. **New KB entries:** what broke, how did she fix, tradeoff, architecture, why no framework, do differently, github. `serveEntry` renders `entry.block`. **New page `/code/`** (registered in deploy.yml, build-site.sh, audit PAGES). **Résumé** rebuilt to brief §7: single column, one-page PDF printed from the page's `@media print` sheet via `scripts/print-resume.mjs` (Playwright), no letter-spaced headings; Smart Benefits cut; skills Engineering / AI / Design. **Studio case study** rewritten with Architecture (inline SVG), Decisions, What broke, Proof (live-index counts), Timeline, Impact; the 0.9 score claim, "brand-trained", "AI training", "25 days", "$2.42M" lead, "~650 × 25 = 16,250" are gone. **Aimee** ownership is the facts.md §5 sentence in the case study and Brix; component library moved from Netlify to `/aimee-ai/components/` (review copy stripped, photo → WebP). **Hub:** NFL (not NFLPA), one revenue label, no "Netflix-style", no "led the design team"; TODO(bri) comments for metric definitions and a research artifact. **About:** experience above hobbies, 2025– and 2016–2025 rows. **Contact:** availability copy per §6, no title list. **Work:** subhead per §4, cards are `<article>`s with View project + Code ↗ / Live ↗ links. `facts.md` at the repo root holds every derived fact and the open TODO(bri) items. Probe 49/50 (50 questions), browser audit 44/44. |
