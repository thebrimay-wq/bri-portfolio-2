...
| 2026-07-19 | **Global Content Studio rewritten from Bri's finalized story.** New narrative built around the localization engine + cost story: Hero (outcome line) → At a glance → Origin (Before/After cards) → The turn (going global, two-col lists) → Solution (4 pillars: brand-trained AI, LLM Wiki, scored localization, human-in-loop) → **Atlas** 8-step pipeline (gate + steps, each with a "why it matters" line) → **The numbers** (4 stat cards + honest cost comparison ~$2.44M vs $21,287 + credibility callout) → Solo build → Trajectory shift (pull quote) → Impact/clients → Learnings → Takeaway. New reusable components: `.beforeafter`/`.ba-card`, `.atlas-gate`/`.atlas-step`/`.atlas-why`, `.stat-cards`, `.compare`/`.compare-card`, `.callout`, `.cs-h2`/`.cs-body`. **Canonical numbers updated site-wide** (homepage KB + `CASE_STUDIES` impact + work-page card): 25 countries · ~650 pieces / 16,250 outputs · 25 days (one country/day) · ~$2.42M avoided (115× cheaper) · built solo with Claude Code — replacing the old "5 locales / a country a week / ~5,800 lines" framing. Only image slot now is `Hero.png` (the study is data/pipeline-driven). |
