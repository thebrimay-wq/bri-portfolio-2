...
| Date | Change |
|---|---|
| 2026-07-28 | **Homepage entry leads with the case studies.** The empty state now mounts the **3 case-study tiles** (`caseCardsHtml()` into `#starter-mount`) instead of the 2×2 recruiter-question grid — Global Content Studio first with a `.featured-pill` — then the composer below, so a visitor sees Bri's work immediately. **Widened Brix's reading column to a true 720px:** `.chat-inner`/`.composer-inner` max-width is now `calc(720px + 2 * var(--px))` (so content inside the gutters is 720, not ~560), `.empty-state` 640→720, `.cs-cards` 560→720. Featured tag row gets `padding-right:80px` so the pill never overlaps tags on narrow cards. |
